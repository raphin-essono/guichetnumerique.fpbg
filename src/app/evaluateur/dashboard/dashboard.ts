...
export class EvaluateurDashboard implements OnInit {
  ...
  stats = computed(() => this.vueEnsemble()?.stats || {
    ...
    sessionsEnCours: 0,
    projetsSessionsActives: 0,
    projetsSessionsActivesEvalues: 0,
    projetsAEvaluer: 0,
    projetsEvalues: 0,
    totalProjets: 0
  });
  ...
  totalProjets = computed(() => this.stats().projetsSessionsActives || 0);
  projetsTermines = computed(() => this.stats().projetsSessionsActivesEvalues || 0);
  projetsEnCours = computed(() => 
    (this.stats().projetsSessionsActives || 0) - (this.stats().projetsSessionsActivesEvalues || 0)
  );
  ...
  // ✅ CORPS : TOUS les projets (de TOUTES les sessions confondues)
  projets = computed(() => {
    const tousProjets = this.vueEnsemble()?.tousProjets || [];
    
    return tousProjets.map(projet => ({
      id: projet.id,
      identifiant: projet.id.substring(0, 8),
      titre: projet.titre,
      budget: 0, // À récupérer depuis les détails du projet si nécessaire
      typeSubvention: '',
      date: '', // Date peut être récupérée depuis sessionNom si nécessaire
      statut: this.mapStatut(projet.statut),
      sessionNom: projet.sessionNom || 'Non spécifié',
  ...
  confirmerDisponibilite(sessionId: string) {
    Swal.fire({
      ...
      text: 'Confirmez-vous votre disponibilité pour cette session ?',
      icon: 'question',
      showCancelButton: true,
      confirmButtonText: 'Oui, je suis disponible',
        ...
        this.dashboardService.confirmerDisponibilite(sessionId, true).subscribe({
          next: (response) => {
            ...
            Swal.fire({
              ...
              text: response.message || 'Vos projets ont été attribués automatiquement.',
              icon: 'success',
              confirmButtonColor: '#10b981',
              timer: 3000,
            ...
            // Recharger les données pour afficher les projets
            this.chargerDonnees();
          },
          error: (err) => {
            ...
            Swal.fire({
              title: 'Erreur',
              text: err.error?.message || 'Erreur lors de la confirmation. Veuillez réessayer.',
              icon: 'error',
  ...
  refuserDisponibilite(sessionId: string) {
    Swal.fire({
      ...
      text: 'Êtes-vous sûr de refuser cette session ? Cette action est définitive.',
      icon: 'warning',
      showCancelButton: true,
        ...
        this.dashboardService.confirmerDisponibilite(sessionId, false).subscribe({
          next: (response) => {
            ...
            Swal.fire({
              ...
              text: response.message || 'Votre refus a été enregistré.',
              icon: 'info',
          ...
          error: (err) => {
            ...
            Swal.fire({
              title: 'Erreur',
              text: err.error?.message || 'Erreur lors du refus. Veuillez réessayer.',
              icon: 'error',
              confirmButtonColor: '#ef4444',
            });
          }
