...
@Component({
  selector: 'app-evaluateur-login',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, RouterLink, HttpClientModule],
  templateUrl: './login.html',
  providers: [Authentifcationservice],
})
export class EvaluateurLogin {
  ...
  async submit() {
    ...
    this.authenticationService
      ...
      .subscribe({
        next: (response) => {
          console.log('✅ Connexion évaluateur réussie - Statut HTTP:', response.status);
          ...
          if (response.body) {
            ...
            if (token) {
              ...
              localStorage.setItem('fpbg.token', token);
            }
            if (user) {
            ...
            const targetUrl =
              redirectTo || (role === 'EVALUATEUR' ? '/evaluateur/dashboard' : '/login');
        ...
        error: (err) => {
          console.error('❌ Erreur de connexion évaluateur:', err);
